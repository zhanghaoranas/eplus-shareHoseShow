...
import Style from "./lease.module.css";
...
export default class Lease extends Component {
	...
	async getFyInfoById() {
		const url = baseUrl + `zulin/zlfy/share?id=${this.state.fyId}`;
	...
	handleClickToInfo() {
		navigate("/lease/info", { state: this.state.fyInfo });
	}
	handleClickToMap() {
	...
	render() {
		...
		if (isLoading) {
			...
			// 标签
			const tagList = [];
			const { ifImg, ifKey, rentType } = fyInfo;
			(ifImg && tagList.push("图片")) ||
				(ifKey && tagList.push("钥匙"))
			tagList.push(rentType);
				...
				<div>
					...
					<section className={`${Style.session} ${Style.bgcolor}`}>
						...
						<div className={Style.activeInfo}>
							<div>
								<span>{fyInfo.price}元/月</span>
								<span>{fyInfo.payWay} {fyInfo.depositWay}</span>
							</div>
							<div>
								<span>
					...
					<section className={`${Style.session} ${Style.bgcolor}`}>
						...
						<ul className={Style.info_list}>
							...
							<li>
								<span>出租类型</span>
								<span>{fyInfo.rentType || "暂无信息"}</span>
							</li>
							<li>
								<span>房源亮点</span>
